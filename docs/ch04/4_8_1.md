...
- ０のとき：(＊ａ)の小数点以下を四捨五入します。
- １のとき：(＊ａ)の小数点以下を切り上げます。
- ２のとき：(＊ａ)に１を足し小数点以下を切り捨てます。切り捨て結果が `v` 設定値を超える場合は `v` 設定値に制限します。
- ３のとき：(＊ａ)の小数点以下を切り捨てます。
...
- ０のとき：(＊ｂ)の小数点以下を四捨五入します。
- １のとき：(＊ｂ)の小数点以下を切り上げます。
- ２のとき：(＊ｂ)に１を足し小数点以下を切り捨てます。切り捨て結果が `vl` 設定値を超える場合は `vl` 設定値に制限します。
- ３のとき：(＊ｂ)の小数点以下を切り捨てます。
...
音量エンベロープ（`@ea`）などによって、発音中（ノートオフ前）に音量が０になった場合、参照されるのは配列の先頭 `V0_MAGNI` の倍率値です。  
`V0_MAGNI` に０以外の倍率（例：`-30dB`）を定義していると、その音量で鳴り続けることになります。

発音中に音量０で完全な無音としたい場合は、`V0_MAGNI` に `0` を定義してください。

※ノートオフ後にエンベロープが終了した場合は、`type` の種別によらず、サンプル精度で無音になります。この動作に `V0_MAGNI` の値は影響しません。
